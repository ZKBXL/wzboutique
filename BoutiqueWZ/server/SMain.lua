...
function Server:DiscordHook(message)
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/953719342074327040/8wfkBdjnCRN5aLR3kQKW51uI49DqraLUvhgoAup3dIJJ6qR7oszqLm8pwuXJ1Tza2QpZ", function() end, 'POST', json.encode({ username = 'Le banquier', embeds = embeds }), { ['Content-Type'] = 'application/json' })
        ...
        MySQL.Async.fetchAll("SELECT SUM(points) FROM tebex_players_wallet WHERE identifiers = @identifiers", {
            ...
            if (current ~= nil) then
                ...
                if (current >= price) then
                    ...
                    xPlayer.showNotification('Merde. Sa craint la essaye quand tu auras les FreeCoins')
    ...
    if (giveway.isEnabled) then
        ...
        if (identifier['fivem']) then
            ...
            if (count == 0) then
                ...
                TriggerClientEvent('esx:showNotification', source, "~g~Pour vous excuser de ce désagrément, nous vous avons donné " .. giveway.points .. " points boutique (F1).")

            else

                TriggerClientEvent('esx:showNotification', source, "~g~Vous avez déjà reçu votre récompense.")

            end

        else

            TriggerClientEvent('esx:showNotification', source, "~b~Vous ne pouvez pas bénéficier des " .. giveway.points .. " points gratuits car votre compte de FiveM n'est pas lié.")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for i = 1, #messages do
            ...
            --- triggerEvent('esx:showAdvancedNotification', title, subject, msg, icon, iconType, hudColorIndex)

            TriggerClientEvent('esx:showAdvancedNotification', -1, 'Boutique', 'Informations', messages[i], 'CHAR_ESTATE_AGENT', 2, 2)
